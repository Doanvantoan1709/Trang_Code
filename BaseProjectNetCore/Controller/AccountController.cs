...
using BaseProject.Core.Email;
using BaseProject.Model.Entities;
using BaseProject.Service.AppUserService;
...
namespace BaseProject.Controllers
{
    ...
    public class AccountController : BaseProjectController
    {
        ...
        {
            var danhMucMatKhauMacDinh = "MATKHAUMACDINH";
            var matKhauMacDinh =  "12345678";
            if (!ModelState.IsValid)
            {
                return DataResponse<string>.False("Lỗi, không thể reset mật khẩu");
            }
        ...
        #endregion
        [HttpGet("IsRole")]
        public async Task<DataResponse<bool>> IsRole(string role)
        {
            if (string.IsNullOrEmpty(role))
            {
                return DataResponse<bool>.False("Mã role không được để trống");
            }
            var getRole = _roleService.FindBy(x => x.Code.Equals(role)).FirstOrDefault();
            if (getRole == null)
            {
                return DataResponse<bool>.False("Role không tồn tại");
            }
            var exists = await _userRoleService.AnyAsync(x => x.UserId == UserId && x.RoleId == getRole.Id);
            return DataResponse<bool>.Success(exists);
        }
    }
