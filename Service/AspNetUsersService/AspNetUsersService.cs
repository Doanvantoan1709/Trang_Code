        ...
        public async Task<PagedList<UserDto>> GetData(AspNetUsersSearch search, UserDto userDto = null)
        {
            ...
            try
            {
                ...
                if (userDto != null && userDto.DonViId != Guid.Empty)
                {
                    ...
                    //if (aspUserRole.GroupRole.Contains(VaiTroConstant.DONVILANHDAO))
                    //{
                    //    query = query.Where(x => !x.vaiTro.Contains(VaiTroConstant.TONGHOP));
                    //}
                }
                ...
                if(userDto != null)
                {
                    var aspUserRole = aspNetUsersRepository.FindBy(x => x.Id == userDto.Id).FirstOrDefault();
                    if (aspUserRole.GroupRole != null)
                    {
                        if (aspUserRole.GroupRole.Contains(VaiTroConstant.DONVILANHDAO))
                        {
                            result.Items = result.Items
                                .Where(x => !x.vaiTro.Contains(VaiTroConstant.TONGHOP))
                                .ToList();
                        }
                    }
                }
