...
namespace BaseProject.Service.DepartmentService
{
    public class DepartmentService : Service<Department>, IDepartmentService
    {
        ...
        public List<DepartmentHierarchy> GetDepartmentHierarchy(bool isParent = false)
        { 
            var result = new List<DepartmentHierarchy>(); 
                var departments = GetQueryable()
                .Where(x=> isParent == false || x.ParentId == null)
        ...
        public List<DepartmentHierarchy> GetDepartmentHierarchyV2(Guid? dvID)
        {
            var result = new List<DepartmentHierarchy>();
            var departments = GetQueryable()
                .Select(d => new DepartmentHierarchy
            ...
            var departmentHierarchy = departments
                .Where(d => (dvID != null && d.ParentId == dvID) || (dvID == null && d.ParentId == null))
                .Select(d => new DepartmentHierarchy
                {
                    Id = d.Id,
                    Title = d.Title,
        ...
        public async Task<List<DropdownOptionTree>> GetDropdownTreeOptionByUserDepartment(bool disabledParent = true, Guid? donViId = null)
        {
            ...
            if (!_cache.TryGetValue(cacheKey, out List<DropdownOptionTree> result))
            {
                ...
                var departments = await GetQueryable()
                    .Where(x => x.IsActive && x.Level <= 1).ToListAsync() ?? new List<Department>();
...
// Thêm các phương thức để xóa cache khi dữ liệu thay đổi
        ...
        public List<Department> getListDepartmentByListId(string litstDepartmentId)
        {
            var listId = litstDepartmentId.Split(',').Select(x => Guid.Parse(x)).ToList();

            if(!listId.Any())
            {
              return new List<Department>();
            }
            return GetQueryable().Where(x => listId.Contains(x.Id)).ToList();
        }
        #endregion
    }
}
