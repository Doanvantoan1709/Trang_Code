...
using BaseProject.Repository.TaiLieuDinhKemRepository;
...
namespace BaseProject.Service.TaiLieuDinhKemService
{
    public class TaiLieuDinhKemService : Service<TaiLieuDinhKem>, ITaiLieuDinhKemService
    {
        ...
        public async Task<List<TaiLieuDinhKem>> GetByIdsAsync(string ids)
        {
            var guidIds = ids.Split(',').Select(x => x.ToGuid()).Where(x => x.HasValue).ToList();
            return await Where(x => guidIds.Contains(x.Id)).ToListAsync();
        }
        public async Task<List<TaiLieuDinhKem>> GetByIdsAsync(List<Guid>? ids)
        {
            if (ids == null || !ids.Any())
            {
                return new List<TaiLieuDinhKem>();
            }
            return await Where(x => ids.Contains(x.Id)).ToListAsync();
        }

        public async Task UpdateItemIdAsync(List<Guid>? ids, Guid itemId)
        {
